...
callEdit(BuildContext context,Item item) async {
  var result = await Navigator.of(context).pushNamed('/save',arguments: item);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                        ...
                        children: [
                          Row(
                            children: [
                                    ...
                                    IconButton(
                                      icon: item.without ?  Icon(Icons.bookmark_border_rounded  ) : Icon(Icons.backspace_rounded   ) ,
                          ...
                          Column(
                            ...
                            children: [
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(item.name ,  overflow: TextOverflow.visible,maxLines: 1 ),
                                  Text("Qtd:" + item.quantity.toString()),
                                  Text(item.note,  overflow: TextOverflow.visible,maxLines: 1 ),
                                ]
                              ),
                            ],
                          ),
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => callEdit(context, Item(    id:Uuid().v4(),    name:"",    note:"",    quantity:1,    caught:false,    without:false) ),
      ),
    );
  }
}